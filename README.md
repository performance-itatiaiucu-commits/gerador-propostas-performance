...
[![Versão](https://img.shields.io/badge/vers%C3%A3o-1.13.2-1F386E)](CHANGELOG.md)
...
- **Tabela de Preços compartilhada (💲)**: botão discreto no canto inferior direito abre a tela com todos os preços do catálogo (busca, contador de alterações e restauração dos padrões). **☁ Publicar para todos** vale em todos os aparelhos em 1–2 minutos; **💾 Só neste aparelho** faz ajuste apenas local.
- **Persistência e sincronização**: salvamento automático do rascunho em edição, histórico de até 40 propostas salvas no navegador, tabela de preços compartilhada entre aparelhos (`precos.json`, com cache offline) e ajustes locais de preço por aparelho.
...
| Tabela de preços compartilhada (vale em todos os aparelhos) | `precos.json` na raiz — publicado pelo botão **☁ Publicar para todos** da tela 💲 (via API do GitHub; exige token com permissão Contents configurado uma vez no aparelho que publica) |
| Ajustes locais de preço (só neste aparelho) | `localStorage` `pop_cat_precos_v1` (+ cache do compartilhado em `pop_cat_precos_shared_v1`) — tela aberta pelo botão 💲 no canto inferior direito |
